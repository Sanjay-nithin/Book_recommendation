...
const GenrePreferences = () => {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      await apiService.updateUserGenrePreferences({ genres: selectedGenres });
      toast({
              ...
              {genres.map((genre) => (
                <div
                  ...
                  className={`
                    ...
                    ${selectedGenres.includes(genre) ? 'border-accent bg-accent/10 shadow-book' : 'border-border hover:border-accent/50 hover:bg-accent/5'}
                  `}
                >
